import { Logo } from "@/components/Logo";
import { LanguageToggle } from "@/components/LanguageToggle";
import { getProjectTracker } from "@/lib/actions/tracker";
import { getProjectMessages } from "@/lib/actions/messages";
import { MessageBox } from "@/components/MessageBox";
import { formatDate } from "@zebraish/lib/format";
import { checkRateLimit } from "@zebraish/lib/rate-limit";
import { getServerT } from "@/lib/i18n/server";
import type { DictKey } from "@/lib/i18n/dictionary";

function statusKey(status: string): DictKey {
  const key = `track.status.${status}` as DictKey;
  return key;
}

export default async function TrackPage({
  searchParams,
}: {
  searchParams: Promise<{ token?: string }>;
}) {
  const { token } = await searchParams;
  const t = await getServerT();
  const withinLimit = token ? await checkRateLimit("track-token", 30, 300) : true;
  const project = token && withinLimit ? await getProjectTracker(token) : null;
  const messages = token && project ? await getProjectMessages(token) : [];
  const rateLimited = Boolean(token) && !withinLimit;

  return (
    <div className="flex min-h-screen flex-col items-center px-6 py-16 bg-bg text-fg">
      <div className="flex w-full max-w-lg items-center justify-between">
        <Logo />
        <LanguageToggle />
      </div>

      <div className="mt-8 w-full max-w-lg rounded-2xl border border-border bg-bg-card p-8">
        <h1 className="mb-6 text-lg font-semibold">{t("track.title")}</h1>

        <form method="get" className="mb-6 flex gap-2">
          <input
            name="token"
            defaultValue={token}
            placeholder={t("track.inputPlaceholder")}
            className="w-full rounded-lg border border-border bg-bg-raised px-3.5 py-2.5 text-fg outline-none focus:border-accent"
          />
          <button
            type="submit"
            className="shrink-0 rounded-lg bg-accent px-4 py-2.5 font-medium text-white transition hover:bg-accent-hover"
          >
            {t("track.submit")}
          </button>
        </form>

        {rateLimited ? (
          <p className="text-sm text-fg-muted">{t("track.rateLimited")}</p>
        ) : token && !project ? (
          <p className="text-sm text-fg-muted">{t("track.notFound")}</p>
        ) : null}

        {project ? (
          <div>
            <div className="mb-6 flex items-center justify-between">
              <div>
                <p className="text-sm font-medium">{project.project_code}</p>
                <p className="text-xs text-fg-muted">{t("track.started", { date: formatDate(project.created_at) })}</p>
              </div>
              <span className="rounded-full border border-border px-3 py-1 text-xs capitalize text-fg-muted">
                {t(statusKey(project.status))}
              </span>
            </div>

            {project.hold_state ? (
              <p className="mb-6 rounded-lg border border-accent/40 bg-bg-raised px-4 py-3 text-sm text-fg">
                {project.hold_state === "on_hold"
                  ? t("track.hold.onHold")
                  : project.hold_state === "awaiting_client"
                    ? t("track.hold.awaitingClient")
                    : t("track.hold.generic")}
              </p>
            ) : null}

            {project.stages.length > 0 ? (
              <div>
                <div className="mb-4">
                  <div className="mb-1.5 flex items-center justify-between text-xs text-fg-muted">
                    <span>{t("track.percentComplete", { percent: project.percent_complete ?? 0 })}</span>
                    {project.current_stage_label ? (
                      <span>{t("track.currentlyStage", { stage: project.current_stage_label })}</span>
                    ) : null}
                  </div>
                  <div className="h-2 w-full overflow-hidden rounded-full bg-bg-raised">
                    <div
                      className="h-full rounded-full bg-accent transition-all"
                      style={{ width: `${project.percent_complete ?? 0}%` }}
                    />
                  </div>
                </div>

                <ul className="flex flex-col divide-y divide-border rounded-lg border border-border">
                  {project.stages.map((stage) => (
                    <li key={stage.stage_order} className="flex items-center gap-3 px-4 py-3">
                      <span
                        className={`flex h-5 w-5 shrink-0 items-center justify-center rounded border text-xs ${
                          stage.status === "complete" ? "border-accent bg-accent text-white" : "border-border"
                        }`}
                      >
                        {stage.status === "complete" ? "✓" : ""}
                      </span>
                      <span className={`text-sm ${stage.status === "complete" ? "text-fg" : "text-fg-muted"}`}>
                        {stage.stage_label}
                      </span>
                    </li>
                  ))}
                </ul>
              </div>
            ) : (
              <p className="text-sm text-fg-muted">{t("track.settingUp")}</p>
            )}

            <div className="mt-6 border-t border-border pt-6">
              <p className="mb-3 text-sm font-medium">{t("track.messages")}</p>
              <MessageBox token={token!} initialMessages={messages} />
            </div>
          </div>
        ) : null}
      </div>
    </div>
  );
}
